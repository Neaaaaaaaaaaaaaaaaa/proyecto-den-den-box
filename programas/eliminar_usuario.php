...
if($id_usuario <= 0){
    echo "<script>alert('Usuario inválido');window.location.href='../html/crear.html';</script>";
    exit;
}
...
try {
    ...
    echo "<script>alert('Usuario eliminado correctamente');window.location.href='../html/crear.html';</script>";

} catch (Exception $e) {
    mysqli_rollback($conexion);
    $msg = addslashes($e->getMessage());
    echo "<script>alert('$msg');window.location.href='../html/crear.html';</script>";
}
