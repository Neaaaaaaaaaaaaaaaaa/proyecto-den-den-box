...
$cargo = mysqli_real_escape_string($conexion, post_value('cargo'));
...
try {
    ...
    if($rol === 3){
        ...
        $res_empleado = mysqli_query($conexion, "SELECT id_empleado FROM EMPLEADOS WHERE id_persona=$id_persona LIMIT 1");
        if($res_empleado && mysqli_num_rows($res_empleado) > 0){
            $id_empleado = intval(mysqli_fetch_assoc($res_empleado)['id_empleado']);
            mysqli_query($conexion, "UPDATE EMPLEADOS SET cargo='$cargo', especialidad='', turno='', estado_laboral='Activo' WHERE id_empleado=$id_empleado");
        } else {
            mysqli_query($conexion, "INSERT INTO EMPLEADOS (id_persona, cargo, especialidad, turno, estado_laboral) VALUES ($id_persona, '$cargo', '', '', 'Activo')");
        }
    ...
    if($es_actualizacion){
        echo "<script>alert('Usuario actualizado correctamente y asignado al inmueble existente');window.location.href='../../html/admin/crear.html';</script>";
    } else {
        echo "<script>alert('Usuario registrado correctamente y asignado al inmueble existente');window.location.href='../../html/admin/crear.html';</script>";
    }
