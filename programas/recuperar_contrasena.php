...
if($accion === 'verificar_correo'){
    ...
    if(!$res || mysqli_num_rows($res) === 0){
        echo "<script>alert('El correo no esta registrado');window.location.href='../html/olvide_contrasena.html';</script>";
        exit();
    }
    ...
    <header class="header">
      <div class="container navbar">
        ...
        <nav class="nav-links">
          ...
          <a href="../html/login.html" class="btn-login">Iniciar Sesion</a>
        </nav>
      </div>
    </header>
...
if($accion === 'actualizar_contrasena'){
    ...
    if($id_usuario <= 0 || $nueva === '' || $confirmar === ''){
        echo "<script>alert('Datos incompletos para actualizar contrasena');window.location.href='../html/olvide_contrasena.html';</script>";
        exit();
    }
    ...
    echo "<script>alert('Contrasena actualizada correctamente');window.location.href='../html/login.html';</script>";
    exit();
}

echo "<script>window.location.href='../html/olvide_contrasena.html';</script>";
exit();
?>
